...
class EditActivity : AppCompatActivity() {
    ...
    private fun insertTodo(){
        realm.beginTransaction()    //Realm 에서 데이터를 추가, 삭제 업데이트 할 때는 beginTransaction() 매서드로 트렌젝션 시작.
                                    ...
                                    //새로운  Todo타입의 Realm 객체 생성
        val newItem = realm.createObject<Todo>(nextId())    //createObject() 메서드로 새로운 Realm 객체 생성
    ...
    private fun updateTodo(id : Long){  //id를 인자로 받음
        ...
        //Realm 객체의 <Todo>타입 객체로부터 데이터를 얻음
        ...
        //값 수정
        updateItem.title = editText.text.toString()    //할 일
    ...
    private fun nextId(): Int {
        ...
        //Todo테이블의 모든 값을 얻기 위해 where<Todo>() 메서드를 사용한다.
        val maxId = realm.where<Todo>().max("id")   //max()메서드는 현재 id중 가장 큰 값을 얻을 때 사용한다.
        if (maxId != null){
